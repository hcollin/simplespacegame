...
function processScrapCommands(commands: Command[], oldGame: GameModel): GameModel {
    ...
    function recycleUnit(g: GameModel, u: ShipUnit): GameModel {
        const recValue = getRecycleProfit(u);
        if (recValue > 0) {
            const unitFaction = { ...getFactionFromArrayById(g.factions, u.factionId) };
        ...
        oldGame.units.forEach((um: ShipUnit) => {
            if (inSameLocation(sm.location, um.location) && sm.ownerFactionId !== um.factionId) {
                ...
                if (systemHasBuilding(sm, BUILDINGTYPE.ORBCANNONS)) {
                    ...
                    invasionTexts.push(
                        `Orbital cannons shot down ${um.troops - afterOrbitalCannon} ${
                            attackingFaction.name
                        } troops while they were landing.`
                    );
        ...
        if (factionTroopCount.size > 0) {
            await asyncMapForeach(factionTroopCount, async (troops: number, factionId: string) => {
                ...
                if (troops > defenses) {
                    ...
                    invasionTexts.push(
                        `Invasion of ${sm.name} by ${attackingFaction.name} from ${ownerName} has failed!`
                    );
                }
...
async function processSystemBuildBuildingCommand(
...
export function spaceCombatPreCombat(
    game: GameModel,
    origCombat: SpaceCombat,
    system: SystemModel | null
): SpaceCombat {
    const combat = { ...origCombat };

    // Deploy Fighters
    const designations: string[] = shuffle([
        "Alpha",
        "Beta",
        "Gamma",
        "Delta",
