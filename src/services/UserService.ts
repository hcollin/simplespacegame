...
import { apiLoadUserByUserId, apiNewUser } from "../api/apiUser";
...
export default function createUserService(serviceId: string, api: JokiServiceApi): JokiService<User | null> {
    ...
    async function setUser(firebaseUser: Firebase.User): Promise<Firebase.User> {
        api.api.setAtom<string>("UserLoginState", "LOGGEDIN");
        const u: User = {
            id: "",
            groups: [],
    ...
    async function populateCurrentUser() {
        if (user !== null) {
            try {
                const fullUser = await apiLoadUserByUserId(user.userId);
                if (fullUser === null) {
                    const newUser = await apiNewUser(user);
					user = newUser;
                } else {
					user = fullUser;
				}
            } catch (e) {
				console.error(`User population failed for ${user.userId} : ${user.name}`);
				console.error(e);
			}
        }
    }
