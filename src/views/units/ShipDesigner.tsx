import { Grid, InputLabel, TextField, Select, MenuItem, Button, createStyles, makeStyles, Theme } from "@material-ui/core";
import { Delete } from "@material-ui/icons";
import { useAtomValue, useSetAtom } from "jokits-react";
...
import { SHIPCLASS, ShipSystem, ShipWeapon, ShipEngine, ShipCustomDesign, ShipDesign } from "../../models/Units";
import { doAddShipDesign, doUpdateShipDesign } from "../../services/commands/FactionCommands";
...
import {
	...
	emptyShipDesign,
} from "../../tools/shipDesigner/shipDesignUtils";
...
type ShipDesignerReducerAction =
	...
	| { type: "FightersMinus" }
	| { type: "ResetDesign" }
	| { type: "LoadDesign"; data: ShipDesign }
...
const shipDesignerReducer: Reducer<ShipDesignerState, ShipDesignerReducerAction> = (
	...
	switch (action.type) {
		case "ResetDesign":
			draft = emptyShipDesign();
			return { ship: { ...draft }, valid: validateShipDesign(draft), id: "" };
		case "LoadDesign":
			console.log("Load Design", action.data);
			draft = convertShipDesignToDesignerModel(action.data);
			return { ship: { ...draft }, valid: validateShipDesign(draft), id: action.data.id };
		case "ChangeName":
			draft.name = action.data;
			draft = shipDesignCalculator(draft);
			return { ship: { ...draft }, valid: validateShipDesign(draft), id: state.id };
		case "RandomName":
...
const ShipDesigner: FC = () => {
	...
	const [{ ship, valid, id }, dispatch] = useReducer<Reducer<ShipDesignerState, ShipDesignerReducerAction>>(shipDesignerReducer, {
		ship: initialDesign,
		valid: false,
		id: "",
	});
	const editableDesign = useAtomValue<ShipDesign | null>("editShipDesign", null);
	const setUnitview = useSetAtom<string>("UnitViewDisplay");
	const faction = useCurrentFaction();

	useEffect(() => {
		if (editableDesign !== null && editableDesign.id !== id) {
			dispatch({ type: "LoadDesign", data: editableDesign });
			return;
		}
		if (editableDesign === null) {
	...
	function saveDesign() {
		if (faction && valid) {
			...
			if (id !== "") {
				design.id = id;
				doUpdateShipDesign(design, faction.id);
			} else {
		...
		<div className={classes.designer}>
			...
			<footer>
				<Button disabled={!valid} variant="contained" color="primary" onClick={saveDesign}>
					{id !== "" && <span>Save design</span>}
					{id === "" && <span>Create design</span>}
				</Button>
			</footer>
		</div>
	);
