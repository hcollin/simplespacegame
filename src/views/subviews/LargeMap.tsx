...
const LargeMap: FC<LargeMapProps> = (props) => {
	...
	const showReset =
		mapRef.current !== null &&
		(Math.abs(mapAdj[0]) > mapRef.current.getAttr("width") * zoomLevel || Math.abs(mapAdj[1]) > mapRef.current.getAttr("height") * zoomLevel);
						...
						{props.systems.map((star: SystemModel) => {
							...
							return (
								<Group key={star.id} x={x} y={y} onClick={() => (isSelected ? deselect() : select(star))}>
									...
									{zoomLevel > 1.3 && (
										<Text text={star.name} fill="#FFF" align="center" opacity={0.8} x={-70} y={10 * zoomLevel} width={140} />
									)}
								</Group>
							);
						})}
...
const CoordinateLine: FC<CoordinateLineProps> = (props) => {
	...
	return (
		<>
            {props.showArrow === true && <Arrow points={[ax, ay, tx, ty]} pointerLength={20*props.zoom} pointerWidth={20*props.zoom} stroke={props.color} strokeWidth={2 * props.zoom} fill={props.color} opacity={0.75} dash={dash}/>}
			{props.showArrow !== true && <Line points={[ax, ay, tx, ty]} dash={dash} stroke={props.color} strokeWidth={2 * props.zoom} />}
		...
		<>
			<Layer>
				{fleets.map((fleet: MapFleet) => {
					...
					const isMyShip = faction && faction.id === fleet.factionId;
					// const inCommand = unitIsInFleet(commands, um);
					...
					if (fleet.state === "MOVING") {
						...
						if (moveFleet.stillInOrbit) {
							const posCor: Coordinates = travelingBetweenCoordinates(moveFleet.location, moveFleet.toCoord, 2);
