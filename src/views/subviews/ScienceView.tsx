			...
			"& > div.page": {
				...
				"& > header": {
					...
					"&>h1": {
						...
						textShadow: "0 0 1rem #BD7, 2px 2px 2px #000, -2px 2px 2px #000, -2px -2px 4px #000,2px -2px 2px #000",
			...
			<PageContainer image={researchimg} color="#BD7" font={faction.style.fontFamily}>
				...
				<div className={classes.row}>
					{faction.technologyFields.map((v: FactionTechSetting) => {
						return <TechnologyFieldWrapper field={v.field} faction={faction} key={v.field} rpTotal={pointsGenerated} />;
					})}
				</div>



				<h2>Technologies ({techs.length} / {DATATECHNOLOGY.length})</h2>

                <FilterButtons values={["ALL", "MEETS PREREQUISITES", "CAN BE RESEARCHED" ]} selected={showMode} onChange={(s: string) => setShowMode(s)}/>
				...
				<div className={classes.technologyGrid}>
					{techs.map((tech: Technology) => {
						...
						if (canAfford && !owned) {
							return (
								<TechCard tech={tech} faction={faction} key={tech.id} className={techClasses} onClick={researchTech} highlightMissing={true} />
							);
                        }
