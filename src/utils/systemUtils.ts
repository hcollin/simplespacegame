...
import { SystemModel, SystemKeyword, Planet } from "../models/StarSystem";
...
export function getSystemEconomy(star: SystemModel, game: GameModel): SystemEconomy {
    ...
    if (star.info) {
        star.info.planets.forEach((p: Planet) => {
            eco.population += p.population;
            eco.maxPopulation += p.maxPopulation;
            eco.foodSupply += p.foodSupply;
            eco.foodProduction += p.foodProduction;
        });
    }
...
export function simulateCommandsEffectsForSystem(
    game: GameModel,
    factionId: string,
    star: SystemModel,
    commands: Command[]
...
function commandIsTargetsSystem(cmd: Command, systemId: string): boolean {
    if (
        [
            CommandType.SystemDefense,
            CommandType.SystemEconomy,
            CommandType.SystemIndustry,
            CommandType.SystemWelfare,
        ].includes(cmd.type)
    ) {
