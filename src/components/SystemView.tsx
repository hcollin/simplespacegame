...
const SystemView: FC = () => {
	...
	function buildUnit(shipD: ShipDesign, system: SystemModel) {
		doBuildUnit(shipD, system.location);
				...
				<div className={classes.mainContent}>
					<Grid container>
						...
						<Grid item lg={7} className={classes.contentArea}>
							<h2>Ship Dock</h2>
							{shipsUnderConstruction.map((ship: ShipDesign, ind: number) => {
								return (
									<div className={`${classes.shipDockSlot} building`} onClick={() => cancelConstruction(ship)}>
										<label>building</label>
										<h2>{ship.type}</h2>

										<p className="ready">Ready in 1 turn(s)</p>
									</div>
								);
							})}
