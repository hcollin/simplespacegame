...
const SystemView: FC = () => {
	...
	const shipsUnderConstruction: ShipUnderConstruction[] = comms.reduce((ships: ShipUnderConstruction[], command: Command) => {
	...
	const allShips: ShipDesign[] = [...DATASHIPS, ...faction.shipDesigns];
					...
					<Grid container>
						...
						<Grid item xs={12} lg={7} className={classes.contentArea}>
							...
							{mySystem &&
								shipsUnderConstruction.map((ship: ShipUnderConstruction) => {
									...
									return (
										<div className={`${classes.shipDockSlot} building cancellable`} onClick={() => cancelConstruction(ship)}>
											<label>building</label>
											<h2>{ship.type}</h2>
											<p className="ready">Ready in {ship.timeLeft} turn(s)</p>
										</div>
						...
						<Grid item xs={12} lg={12} className={classes.contentArea}>
							...
							<div className={classes.slots}>
								...
								{buildingUnderConstruction && !buildingUnderConstruction.cancellable && (
									<BuildingDesignSlot building={buildingUnderConstruction} underConstruction={buildingUnderConstruction.turnsLeft} />
								)}
									...
									if (slot === null) {
										return (
											<div className={classes.buildingSlot} key={`bslot-${index}`}>
												...
												{mySystem && (
													<Button className="fullSizeButton" onClick={() => setBuildingView("building")}>
														+
													</Button>
												)}
											</div>
