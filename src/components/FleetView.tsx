...
const SelectUnitToFleet: FC<ContentProps> = (props) => {
    ...
    return (
        <>
            {/* <h4>Fleet</h4> */}
			{props.units.length > paginationLimit && <div className="pagination">
				<Button variant="outlined" disabled={unitIndex === 0} onClick={() => setUnitIndex((prev: number) => Math.max(0, prev - paginationLimit))}><NavigateBeforeIcon /></Button>
				<div>
					{unitIndex + 1} - {Math.min(unitIndex + paginationLimit, props.units.length)} / {props.units.length}
				</div>
				<Button variant="outlined" disabled={unitIndex + paginationLimit >= props.units.length} onClick={() => setUnitIndex((prev: number) => Math.min(prev + paginationLimit, props.units.length - 1)) }><NavigateNextIcon /></Button>
			</div>}
            <div className="units">
			...
			{!props.system && slowestUnit && <HelpContainer><p>Select destination system from the map.</p></HelpContainer>}
        </>
    );
};
